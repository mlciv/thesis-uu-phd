\section[Dialogue and Schema Modeling]{Dialogue and Schema Modeling}
...
%sentences need to be encoded. Extending to our schema-guided dialogue
%task, the pair of text are dialogue context and schema
...
%context dialogue encoding, Poly-Encoder borrow the cross attention
%ability from Cross-Encoder, it first predicts $m$ different
%representation vector for dialogue context rather than a single vector
%in Dual-Encoder, then it attend over them using the precomputed single
%schema as a query, finally sum up all the weighted dialogue context as
%schema-aware dialogue representation.
...
%Especially, slots from previous dialogue state can be carried over to
%the next dialogue state, cross the same or different dialogue state. For
...
%for those common dialogue state tracking strategies such as two-stage,
...
Categorical slots have a set of candidate values. We cannot predict
...
%including using adopting dialogue state from system turn, hand-crafted
...
% More details of the performance on each subtasks are shown in \autoref{tbl:encoder-speed}.
