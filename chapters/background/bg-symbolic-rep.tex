...
\Paragraph{Anchoring} Before we introduce each semantic
representation, we first define the term \kw{anchoring} used to
organize them. As the same definition of \kw{anchoring} used in the
...
\kw{sentence-anchoring}.

This thesis covers broad-coverage graph-based semantic representations
in a single sentence, which involves \kw{lexical-anchoring} and
...
Then \citet{Iva:Oep:Ovr:12} propose a two-stage version to transformer
