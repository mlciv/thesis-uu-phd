...
\item \textbf{Anchoring:} As the same definition of \kw{anchoring}
  ...
  as ~\kw{anchoring}~(of nodes onto substrings); other commonly used
...
\kw{lexical-anchoring} and \kw{phrasal-anchoring} representations. For lexical
...
%\subsubsection{Bilexical Semantic Dependencies}
%\label{ssec:bg:bileixcal}
...
whole, and it uses BV (bound variable, \eg, the word \tquoted{a}) as
...
keywords~(\tquoted{date-entity,}~\tquoted{person,}~and~\tquoted{name,}
etc.), quantities~(\tquoted{temporal-quantity}~and~\tquoted{distance-quantity,}~etc.), and logical
conjunctions~(\tquoted{and,}~etc).  AMR strives for a more logical,
less syntactic representation. For example, it represents the
word~\tquoted{nonexecutive} with a negation~\tquoted{(:polarity -)}
...
nonterminal node corresponding to~\tquoted{Pierre, Vinken, 61 years
...
nonterminal nodes, prefixed with \tquoted{IN:}~or
~\tquoted{SL:} labels, respectively.
