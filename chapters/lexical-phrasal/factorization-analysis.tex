\section[Related Work and Anchoring Analysis]{Related Work and
  Anchoring Analysis}
...
subwords, e.g., the whole graph fragment \tquoted{(p/possible-01
  :polarity -)} is evoked by word ``impossible", where the subword
\tquoted{im} actually evoked the relation polarity and concept
\tquoted{-}; On the other side, sometimes concepts are evoked by
multiple words, e.g., named entities, \tquoted{(c/city :name (n/name
  :op1 `New' :op2 `York'))}, which also happens in explict anchoring
